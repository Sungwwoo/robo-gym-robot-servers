...
class RosBridge:
    def __init__(self, real_robot=False):
        ...
        self.max_lin_vel, self.min_lin_vel = (
            self.apf_utils.get_max_lin_vel(),
            self.apf_utils.get_min_lin_vel(),
        )
        self.max_ang_vel, self.min_ang_vel = (
            self.apf_utils.get_max_ang_vel(),
            self.apf_utils.get_min_ang_vel(),
        )

        self.apf_init_kp, self.apf_init_eta = (
            self.apf_utils.init_kp,
            self.apf_utils.init_eta,
        )
...
class RosBridge_with_PD(RosBridge):
    ...
    def get_state(self):
        ...
        rospy.loginfo("Collecting informations from gazebo")
        # Get states
        target = copy.deepcopy(self.target)
        base_scan = copy.deepcopy(self.scan)
        base_pose = copy.deepcopy(self.base_pose)
        ...
        rospy.loginfo("Generating state msg")
        ...
        rospy.loginfo("Successfully generated state msg with length %d", len(msg.state))

        return msg

    # TODO: Set robot position, target pose, and weights of APF
    def set_state(self, state_msg):
        """Set states using states of reset()
        Args:
            state_msg:
    ...
    def set_params(self, KP, ETA, ratio, linear_kp, angular_kp, angular_kd):
        rospy.sleep(0.07)
        self.apf.set_weights(KP, ETA)
        self.apf.set_num_obstacles_ratio(ratio)
