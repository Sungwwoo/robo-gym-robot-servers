...
from apf_robot_server.basic_apf_ros_bridges import RosBridge_with_PD
import grpc
import rospy
...
class RobotServerServicer(robot_server_pb2_grpc.RobotServerServicer):
    def __init__(self, real_robot):
        self.rosbridge = RosBridge_with_PD(real_robot=real_robot)
    ...
    def SendAction(self, request, context):
        try:
            self.rosbridge.set_params(request.action[0], request.action[1], request.action[2], request.action[3], request.action[4])
            return robot_server_pb2.Success(success=1)
        except:
            return robot_server_pb2.Success(success=0)
