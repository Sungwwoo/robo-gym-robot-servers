...
if __name__ == "__main__":
    ...
    best_time = 1000
    f_summary = open("logs_move_base/room" + str(room) + "/experiment_summary.txt", "w")

    # Used for move_base experiment
    relative_target = [a - b for a, b in zip(target_points[room], start_points[room])]

    clear_costmap_srv = rospy.ServiceProxy("move_base/clear_costmaps", Empty)
    for iteration in range(0, runs):
        ...
        # Open a new log file
        fileName = (
            "logs_move_base/room" + str(room) + "/experiment_run_" + str(iteration + 1) + ".txt"
        )
        ...
        while calcDistance(relative_target[0:2], base_pose[0:2]) > 0.3:
            rospy.loginfo(
