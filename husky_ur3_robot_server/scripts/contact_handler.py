...
class ContactHandler:
    def __init__(self):
        ...
        self.sub_collision_3 = message_filters.Subscriber("base_collision_3", ContactsState)
        ts = message_filters.TimeSynchronizer(
            [
                self.sub_collision_0,
                self.sub_collision_1,
                self.sub_collision_2,
                self.sub_collision_3,
            ],
            10,
        )
    ...
    def cbCollision(self, data1, data2, data3, data4):
        info = Bool()
        if not (data1.states == [] and data2.states == [] and data3.states == [] and data4.states == []):
            info.data = True
        else:
            info.data = False
...
if __name__ == "__main__":
    rospy.init_node("husky_contact_hander")
    p = ContactHandler()
